...
DB_PATH = Path(DATA_DIR) / "data" / "hbc.db"
BACKUP_CONFIG_PATH = Path(DATA_DIR) / "data" / "backup_config.json"
DEFAULT_CONFIG = {
    # Default only — a saved data/backup_config.json always wins over this.
    "local_path": str(Path(DATA_DIR) / "backups"),
    "keep_count": 7,
    "rclone_remote": "",
    "rclone_path": "hbc-backup",
}
...
def _rotate_backups(backup_dir: Path, keep: int):
    files = sorted(backup_dir.glob("hbc_backup_*.zip"), key=lambda f: f.stat().st_mtime)
...
def restore_zip_bytes(data: bytes) -> dict:
    ...
    names = zf.namelist()
    if "hbc.db" not in names:
        raise HTTPException(400, "ZIP does not contain hbc.db — not a valid backup")

    # Restore DB via sqlite3 backup API (safe while service is running)
    with tempfile.NamedTemporaryFile(suffix=".db", delete=False) as tmp:
        tmp.write(zf.read("hbc.db"))
        tmp_path = tmp.name
...
def save_backup_zip(cfg: dict) -> Path:
    ...
    out = backup_dir / f"hbc_backup_{ts}.zip"
    buf = io.BytesIO()
    _build_zip(buf)
...
def backup_list(request: Request):
    ...
    if not backup_dir.exists():
        return []
    files = sorted(backup_dir.glob("hbc_backup_*.zip"), key=lambda f: f.stat().st_mtime, reverse=True)
    return [
        {
            "filename": f.name,
